    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _, custom_res_dict = validate.run(data_dict,
                                                batch_size=batch_size // WORLD_SIZE * 2,
                                                imgsz=imgsz,
                                                iou_thres=hyp['iou_t'],
                                                conf_thres=hyp['conf_t_'],
                                                half=amp,
                                                model=ema.ema,
            ...
            callbacks.run('on_fit_epoch_end', log_vals, epoch, best_fitness, fi, custom_res_dict=custom_res_dict)
    ...
    if RANK in {-1, 0}:
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
                    results, _, _, custom_res_dict = validate.run(
                        data_dict,
                        batch_size=batch_size // WORLD_SIZE * 2,
                        imgsz=imgsz,
                        model=attempt_load(f, device).half(),
                        iou_thres=0.65 if is_coco else hyp['iou_t'],  # best pycocotools at iou 0.65
                        conf_thres=hyp['conf_t_'],
                        single_cls=single_cls,
                        dataloader=val_loader,
                        save_dir=save_dir,
                    ...
                    if is_coco:
                        callbacks.run('on_fit_epoch_end', list(mloss) + list(results) + lr, epoch, best_fitness, fi, custom_res_dict=custom_res_dict)
